...
public class SubmachineBase : WeaponBase, ISerializationCallbackReceiver
{
    [Header("View")]
    public Sprite Icon;
    ...
    public override void Init(EcsWorld world, BattleState state)
    {
        ...
        if (state.TryGetEntity("player", out int playerEntity))
        {
            ...
            ref var autorifleComp = ref world.GetPool<SubmachineComponent>().Add(weaponEntity);
    ...
    public override void Upgrade(EcsWorld world, BattleState state)
    {
        if (state.TryGetEntity("player", out int playerEntity))
        {
            ...
            {
                if (world.GetPool<SubmachineComponent>().Has(weaponEntity))
                {
