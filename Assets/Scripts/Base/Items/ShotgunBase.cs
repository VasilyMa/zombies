...
[CreateAssetMenu(fileName = "ShotgunBase", menuName = "Scriptable Objects/ShotgunBase")]
public class ShotgunBase : WeaponBase, ISerializationCallbackReceiver
{
    [Header("View")]
    public Sprite Icon;
    ...
    public override void Init(EcsWorld world, BattleState state)
    {
        ...
        if (state.TryGetEntity("player", out int playerEntity))
        {
            ...
            ref var handgunComp = ref world.GetPool<ShotgunComponent>().Add(weaponEntity);
            ...
            ref var missileCountComp = ref world.GetPool<CountMissileComponent>().Add(weaponEntity);
            missileCountComp.Count = levelData.MissileCount;

            ref var hitComp = ref world.GetPool<HitEffectComponent>().Add(weaponEntity);
            hitComp.Prefab = HitPrefab;
    ...
    public override void Upgrade(EcsWorld world, BattleState state)
    {
        if (state.TryGetEntity("player", out int playerEntity))
        {
            ...
            {
                if (world.GetPool<ShotgunComponent>().Has(weaponEntity))
                {
    ...
    public override void Upgrade(EcsWorld world, BattleState state, int entity)
    {
        ...
        if (nextLevel < Levels.Count)
        {
            ...
            ref var missileCountComp = ref world.GetPool<CountMissileComponent>().Get(weaponEntity);
            missileCountComp.Count = levelData.MissileCount;
        }
    }
