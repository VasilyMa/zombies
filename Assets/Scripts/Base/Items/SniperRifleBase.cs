...
public class SniperRifleBase : WeaponBase, ISerializationCallbackReceiver
{
    [Header("View")]
    public Sprite Icon;
    ...
    public override void Init(EcsWorld world, BattleState state)
    {
        ...
        if (state.TryGetEntity("player", out int playerEntity))
        {
            ...
            ref var handgunComp = ref world.GetPool<SniperRifleComponent>().Add(weaponEntity);
    ...
    public override void Upgrade(EcsWorld world, BattleState state)
    {
        if (state.TryGetEntity("player", out int playerEntity))
        {
            ...
            {
                if (world.GetPool<SniperRifleComponent>().Has(weaponEntity))
                {
